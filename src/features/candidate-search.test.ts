...
const filters = { organization_id: "company-id", school_id: "school-id", match: "all" as const, skills: [{ skill_id: "typescript", min_level: 7 }], verified_only: true, require_shared_project: true };

test("candidate search snapshots filters and preserves them across pages", () => {
  const first = submitSearch(filters);
  filters.skills[0]!.min_level = 1;
...
test("candidate criteria reject invalid levels, duplicates, and backend-limit overflow", () => {
  ...
  expect(() => pageFor(submitSearch(filters), 0)).toThrow("positive whole number");
});
